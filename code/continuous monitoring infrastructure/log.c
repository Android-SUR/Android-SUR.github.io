...
{
	log_proc = proc_mkdir("log", NULL);
	if (!log_proc) {
		pr_err("failed to create log node.\n");
		goto failed_to_create_sysfs;
	}

	delay_entry = proc_create("delay", S_IRUGO,
			log_proc, &proc_delay_file_operations);

	if (task_node_init()) {
		binder_delay_entry = proc_create("binder_delay", S_IRUGO | S_IWUGO,
				log_proc, &proc_binder_start_file_operations);
