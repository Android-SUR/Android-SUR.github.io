#define pr_fmt(fmt) "EXCEPTION_LOG: " fmt
...
#define EXCEPTION_LOG_DEV_NAME      "exception_log"

static struct exception_log_dev exception_log_dev;
...
static ssize_t exception_log_read(struct file *fp, char __user *buff, size_t length, loff_t *ppos)
{
	...
	struct exception_log_dev *tmp_exception_log_dev = &exception_log_dev;
...
static int __init exception_log_init(void)
{
	...
	pr_info("exception_log init ok");
	return 0;
}

module_init(exception_log_init);

static void __exit exception_log_exit(void)
{
	struct exception_log_dev *tmp_exception_log_dev = &exception_log_dev;
...
module_exit(exception_log_exit);

MODULE_DESCRIPTION("exception log device driver");
